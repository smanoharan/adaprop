...
%% Visualisation of all candidate bisecting hyperplanes:
\begin{figure}
\begin{center}
        ...
        \end{subfigure}
\end{center}
\caption{Candidate bisections generated by each method for $\mcl{M}$ of the example dataset}
\label{visCandGen}
\end{figure}

\subsubsection{Selecting the optimal bisection}
\label{secOptBisection}

Given a set of candidate bisections $B$ for the labelled set of instances $\mcl{M}_i$, 
    \AdaProp aims to find the optimal bisection $B_i$ in $B$ such that
    the training set error on $\mcl{D}$ using the base learner is minimized.
For each candidate bisection $B_j$,
...
The candidate bisection which results in the least training set 
    error during this process is selected as the 
    optimal bisection.
This process is given in Algorithm~\ref{algoOptBisector}.

\begin{algorithm}
\caption{Finding the Optimal Bisection}
\label{algoOptBisector} 
\begin{algorithmic}
    \Function{EvaluateBisection}{$T,n_i,B_j$}
    ...
    \EndFunction
    \State 
    \Function{FindOptimalBisection}{$T,n_i,B$}
